...
bool Solver::solve(Position p)
{
    if (p.getRow() > maze->num_rows() or p.getCol() > maze->num_cols()
        or p.getRow() < 0 or p.getCol() < 0) {
        return false;
    }
    if (maze->marker_at(p) == WALL or maze->marker_at(p) == PATH
        or maze->marker_at(p) == REJECT) {
            return false;
        }
    ...
    if (solve(p.north()) or solve(p.east()) or solve(p.south()) or solve(p.west())) return true;
