...
void Heap::downHeap(int location) {
    if(leftChild(location) >= heapSize && rightChild(location) >= heapSize){ //base case
        return;
    }
    if(data[leftChild(location)] < data[rightChild(location)]){
        if(data[location] > data[leftChild(location)]){
            string placeholder = data[location];
            data[location] = data[leftChild(location)];
            data[leftChild(location)] = placeholder;
            downHeap(leftChild(location));
        }
    }

    else if(data[location] > data[rightChild(location)]){
        string placeholder = data[location];
        data[location] = data[rightChild(location)];
        data[rightChild(location)] = placeholder;
